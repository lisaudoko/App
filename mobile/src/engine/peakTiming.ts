 ...
 */
export function estimatePeakTiming(logs: WeeklyLog[]): PeakTimingResult {
  ...
  const { slopePerWeek } = buildTrajectory(logs, 1);
  ...
  detail.push(slopePerWeek > 0.02 ? 'Form improving this block' : slopePerWeek < -0.02 ? 'Form dipping this block' : 'Form holding steady');

  if (slopePerWeek > 0 && rpeTrend <= 0.6) {
